...
export default function StepParticipantType({ errors = {} }: Props) {
  ...
  return (
    <div className="glass-panel p-8 md:p-10 rounded-2xl w-full animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <p className="text-text-muted mb-10 font-inter text-lg">Select how you are registering for Eureka.</p>
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div 
          ...
          className={`flex-1 rounded-2xl p-8 transition-all cursor-pointer text-center border relative overflow-hidden group ${
            participantType === 'student' 
              ...
              : 'bg-text-dark border-white/5 hover:border-white/15 hover:bg-surface-secondary/50'
          }`}
        >
          {/* Active indicator dot */}
          <div className={`absolute top-4 right-4 w-3 h-3 rounded-full transition-all ${
            participantType === 'student' ? 'bg-primary shadow-[0_0_10px_#1A6FF5]' : 'bg-white/10'
          }`}></div>

          <div className={`w-20 h-20 rounded-full mx-auto mb-6 flex items-center justify-center border transition-all duration-300 ${
            participantType === 'student' ? 'bg-primary/10 border-primary/50 text-primary shadow-[0_0_20px_rgba(26,111,245,0.2)]' : 'bg-surface-secondary border-white/5 text-gray-400 group-hover:text-white'
          }`}>
            <User size={32} strokeWidth={1.5} />
          </div>
          <h3 className="text-xl font-bold text-white mb-3 font-poppins">Student</h3>
          <p className="text-sm text-text-muted font-inter leading-relaxed">I am registering as an individual or team of students with an idea.</p>
        </div>
        ...
        >
          {/* Active indicator dot */}
          <div className={`absolute top-4 right-4 w-3 h-3 rounded-full transition-all ${
            participantType === 'startup' ? 'bg-primary shadow-[0_0_10px_#1A6FF5]' : 'bg-white/10'
          }`}></div>

          <div className={`w-20 h-20 rounded-full mx-auto mb-6 flex items-center justify-center border transition-all duration-300 ${
            participantType === 'startup' ? 'bg-primary/10 border-primary/50 text-primary shadow-[0_0_20px_rgba(26,111,245,0.2)]' : 'bg-surface-secondary border-white/5 text-gray-400 group-hover:text-white'
          }`}>
            <Rocket size={32} strokeWidth={1.5} />
          </div>
          <h3 className="text-xl font-bold text-white mb-3 font-poppins">Startup</h3>
          <p className="text-sm text-text-muted font-inter leading-relaxed">We are registering as an established startup.</p>
        </div>
      </div>
    </div>
  );
